...
// create a component
const QnaUpdateForm = ({ route }) => {
    const { oldEventData } = route.params;
    const [nowDate, setNowDate] = useState(new Date())
    const navigation = useNavigation();
    const [date, setDate] = useState(new Date(moment(oldEventData.event_date).format()));
    ...
    const [startDate, setStartDate] = useState(new Date(moment(oldEventData?.registration_start_date).format()));
    const [endDate, setEndDate] = useState(new Date(moment(oldEventData?.registration_end_date).format()));
    ...
    console.log(moment(oldEventData.event_date).format())
    console.log(nowDate)
    //form data
    const { control, handleSubmit, setValue, formState: { errors, shouldValidate } } = useForm({
        defaultValues: {
            ...
            description: oldEventData.description,
            event_date: new Date(oldEventData.event_date),
            start_time: new Date(`2024-07-18T${oldEventData?.start_time}`),
            end_time: new Date(`2024-07-18T${oldEventData?.end_time}`),
            registration_start_date: new Date(oldEventData.registration_start_date),
            registration_end_date: new Date(oldEventData.registration_end_date),
            fee: `${oldEventData.fee}`,
            max_time: `${oldEventData.max_time}`,
            min_time: `${oldEventData.min_time}`,
            time_interval: `${oldEventData.time_interval}`,
    ...
    const onSubmit = data => {
        ...
        if (uploadStatus.afterSubmit) {
            ...
            axios
                .post(AppUrl.QnaUpdate, data, axiosConfig)
                .then(res => {
    ...
    return (
        <>
            ...
            <Text style={{color: '#fff'}}>Learning Session Title</Text>
                        ...
                        <View>
                            <View>
                                <Text style={styles.title}>Title Qna form</Text>
                            </View>
                            <Controller
                                control={control}
                                rules={{
              ...
              <Text style={{ color: '#9e9e9e', paddingLeft: 8, fontSize: 13 }}>Upload File</Text>
                        ...
                        <View style={{ marginVertical: 8 }}>
                            ...
                            {error.event_date && <Text style={{ color: 'red', marginLeft: 10 }}>This is required.</Text>}
                        </View>
                        ...
                        <View style={{ marginVertical: 8 }}>
                            ...
                            <Controller
                                ...
                                name="time_interval"
                            />
                            {errors.time_interval && <Text style={{ color: 'red', marginLeft: 10, marginTop: -7 }}>This is required.</Text>}
                        </View>
